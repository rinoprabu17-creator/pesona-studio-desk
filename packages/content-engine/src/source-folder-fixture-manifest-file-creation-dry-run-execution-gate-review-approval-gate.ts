import { z } from "zod";
import {
  parseAndReviewSourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewFixture,
  type FixtureManifestFileCreationDryRunExecutionGateReviewItem,
  type SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewOutput
} from "./source-folder-fixture-manifest-file-creation-dry-run-execution-gate-review.ts";

const EXECUTION_GATE_REVIEW_APPROVAL_GATE_ID =
  "phase-2j31-source-folder-fixture-manifest-file-creation-dry-run-execution-gate-review-approval-gate";
const FUTURE_FIXTURE_MANIFEST_FILE_CREATION_DRY_RUN_EXECUTION_GATE_APPROVAL_REVIEW_SCOPE =
  "future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review_only";
const SAFE_FIXTURE_ROOT = "packages/content-engine/fixtures/read-only-intake-sample/";

const ExecutionGateReviewApprovalActionSchema = z.enum([
...
const ExecutionGateReviewApprovalStatusSchema = z.enum([
...
export const SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecordSchema = z.object({
  ...
  requested_actions: z.array(ExecutionGateReviewApprovalActionSchema).default([])
}).strict();

export const SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixtureSchema = z.object({
  fixture_version: z.literal(
    "source-folder-fixture-manifest-file-creation-dry-run-execution-gate-review-approval-gate-smoke-v1"
  ),
  mode: z.literal("controlled_fixture_manifest_file_creation_dry_run_execution_gate_review_approval_gate"),
  ...
  execution_gate_review_approval_records: z.array(
    SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecordSchema
  ).min(1),
  notes: z.string().min(1)
}).strict();

export const FixtureManifestFileCreationDryRunExecutionGateReviewApprovalItemSchema = z.object({
  execution_gate_review_item_id: z.string().min(1),
  ...
  source_execution_gate_review_status: z.enum([
  ...
  approved_at: z.string().min(1).nullable(),
  approval_scope: z.string().min(1).nullable(),
  approval_status: ExecutionGateReviewApprovalStatusSchema,
...
export const SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateOutputSchema = z.object({
  fixture_manifest_file_creation_dry_run_execution_gate_review_approval_gate_id: z.string().min(1),
  source_id: z.string().min(1),
  source_label: z.string().min(1),
  source_root: z.string().min(1),
  dry_run: z.boolean(),
  read_only: z.boolean(),
  source_execution_gate_review_status: z.enum(["execution_gate_review_ok", "execution_gate_review_denied"]),
  reviewed_execution_gate_review_items: z.number().int().min(0),
  approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review:
    z.number().int().min(0),
  needs_owner_review: z.number().int().min(0),
  incomplete_approval: z.number().int().min(0),
  blocked_approval: z.number().int().min(0),
  approval_items: z.array(FixtureManifestFileCreationDryRunExecutionGateReviewApprovalItemSchema),
...
export type SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecord = z.infer<
  typeof SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecordSchema
>;
export type SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixture = z.infer<
  typeof SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixtureSchema
>;
export type FixtureManifestFileCreationDryRunExecutionGateReviewApprovalItem = z.infer<
  typeof FixtureManifestFileCreationDryRunExecutionGateReviewApprovalItemSchema
>;
export type SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateOutput = z.infer<
  typeof SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateOutputSchema
>;

export type SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateBatchResult = {
  provider: "fake_content_engine";
  fixture: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixture;
  gates: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateOutput[];
  reviewedExecutionGateReviewItems: number;
  approvedForFutureFixtureManifestFileCreationDryRunExecutionGateApprovalReviewCount: number;
...
export function parseSourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixture(
  input: unknown
): SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixture {
  return SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixtureSchema.parse(input);
}

export function parseAndReviewSourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixture(input: {
...
}): SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateBatchResult {
  const fixture = parseSourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateFixture(
    input.approvalGateFixtureInput
  );
  const executionGateReviewResult =
    parseAndReviewSourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewFixture({
  ...
  const gates = executionGateReviewResult.reviews.map((review) =>
    reviewExecutionGateReviewApprovalGate(review, fixture.execution_gate_review_approval_records)
  );
  ...
  return {
    ...
    fixture,
    gates,
    reviewedExecutionGateReviewItems: sum(gates.map((gate) => gate.reviewed_execution_gate_review_items)),
    approvedForFutureFixtureManifestFileCreationDryRunExecutionGateApprovalReviewCount: sum(
      gates.map((gate) =>
        gate.approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review
      )
    ),
...
export function reviewExecutionGateReviewApprovalGate(
  executionGateReview: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewOutput,
  approvalRecords: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecord[]
): SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateOutput {
  const sourceStatus = executionGateReview.execution_gate_review_ok > 0
    ? "execution_gate_review_ok"
    : "execution_gate_review_denied";
  const duplicateIds = new Set(
    executionGateReview.execution_gate_review_item_reviews
      .filter((item) => item.blocking_reasons.includes("duplicate_suggested_footage_id_blocked"))
      .map((item) => item.suggested_footage_id)
  );
  const approvalItems = sourceStatus === "execution_gate_review_ok"
    ? executionGateReview.execution_gate_review_item_reviews.map((item) =>
        approvalItemForExecutionGateReview(
          item,
          approvalRecords.find((record) => record.execution_gate_review_item_id === item.execution_gate_review_item_id),
          duplicateIds,
          executionGateReview
        )
      )
  ...
  return SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalGateOutputSchema.parse({
    fixture_manifest_file_creation_dry_run_execution_gate_review_approval_gate_id:
      `${EXECUTION_GATE_REVIEW_APPROVAL_GATE_ID}-${executionGateReview.source_id}`,
    source_id: executionGateReview.source_id,
    source_label: executionGateReview.source_label,
    source_root: executionGateReview.source_root,
    dry_run: executionGateReview.dry_run,
    read_only: executionGateReview.read_only,
    source_execution_gate_review_status: sourceStatus,
    reviewed_execution_gate_review_items: sourceStatus === "execution_gate_review_ok"
      ? executionGateReview.execution_gate_review_item_reviews.length
      : 0,
    approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review: approvedCount,
    ...
    approval_policy_findings: approvalPolicyFindings(sourceStatus, executionGateReview),
    ...
    target_fixture_manifest_path:
      approvalItems.find(
        (item) =>
          ...
          "approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review"
      )?.target_fixture_manifest_path ?? executionGateReview.target_fixture_manifest_path,
    metadata_write_allowed: false,
    manifest_write_allowed: false,
    fixture_manifest_write_allowed: executionGateReview.fixture_manifest_write_allowed,
    fixture_manifest_file_creation_gate_allowed: executionGateReview.fixture_manifest_file_creation_gate_allowed,
    fixture_manifest_file_creation_dry_run_allowed: executionGateReview.fixture_manifest_file_creation_dry_run_allowed,
    fixture_manifest_file_creation_execution_gate_allowed:
      executionGateReview.fixture_manifest_file_creation_execution_gate_allowed,
    fixture_manifest_file_creation_dry_run_execution_review_allowed:
      executionGateReview.fixture_manifest_file_creation_dry_run_execution_review_allowed,
    fixture_manifest_file_creation_dry_run_execution_gate_allowed:
      executionGateReview.fixture_manifest_file_creation_dry_run_execution_gate_allowed,
    fixture_manifest_file_creation_dry_run_execution_gate_review_allowed: approvedCount > 0,
...
function approvalItemForExecutionGateReview(
  item: FixtureManifestFileCreationDryRunExecutionGateReviewItem,
  record: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecord | undefined,
  duplicateIds: Set<string>,
  executionGateReview: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewOutput
): FixtureManifestFileCreationDryRunExecutionGateReviewApprovalItem {
  const approvalRecord = record ?? defaultApprovalRecord(item.execution_gate_review_item_id);
  const missingApprovalFields = missingApprovalFieldsFor(approvalRecord);
  const blockingReasons = blockingReasonsFor(item, approvalRecord, duplicateIds, executionGateReview);
  const status = approvalStatusFor(item, approvalRecord, missingApprovalFields, blockingReasons);

  return FixtureManifestFileCreationDryRunExecutionGateReviewApprovalItemSchema.parse({
    ...
    source_execution_gate_review_status: item.execution_gate_review_status,
    ...
    review_notes: [
      `Source execution gate review status: ${item.execution_gate_review_status}.`,
      `Fixture manifest file creation dry-run execution gate review approval status: ${status}.`,
...
): SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecord {
  return {
...
function missingApprovalFieldsFor(
  record: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecord
): string[] {
  ...
  if (
    ...
    missing.push("approval_scope_future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review_only");
  }
  return missing;
}
...
function blockingReasonsFor(
  ...
  if (executionGateReview.source_root !== SAFE_FIXTURE_ROOT) reasons.push("source_root_not_safe_fixture");
  if (!executionGateReview.dry_run) reasons.push("dry_run_false_blocked");
  if (!executionGateReview.read_only) reasons.push("read_only_false_blocked");
  if (executionGateReview.in_memory_execution_gate_review_preview.persisted) {
    reasons.push("in_memory_execution_gate_review_preview_persisted_blocked");
  }
  if (item.execution_gate_review_status === "blocked_execution_gate_review") {
    reasons.push("source_execution_gate_review_blocked_cannot_be_upgraded");
  }
  if (item.execution_gate_review_status === "incomplete_execution_gate_review") {
    reasons.push("source_execution_gate_review_incomplete_cannot_be_upgraded");
  }
  if (duplicateIds.has(item.suggested_footage_id)) reasons.push("duplicate_suggested_footage_id_blocked");
  for (const reason of item.blocking_reasons) reasons.push(`source_execution_gate_review_${reason}`);
...
): z.infer<typeof ExecutionGateReviewApprovalStatusSchema> {
  if (item.execution_gate_review_status === "incomplete_execution_gate_review") return "incomplete_approval";
  if (blockingReasons.length > 0) return "blocked_approval";
  if (item.execution_gate_review_status === "needs_owner_review") return "needs_owner_review";
  if (missingApprovalFields.length > 0) {
    if (item.execution_gate_review_status === "execution_gate_review_ok" && !record.approval_record_found) {
      return "needs_owner_review";
    }
    return "incomplete_approval";
  }
  if (item.execution_gate_review_status !== "execution_gate_review_ok") return "blocked_approval";
  return "approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review";
}

function approvalPolicyFindings(
  sourceStatus: "execution_gate_review_ok" | "execution_gate_review_denied",
  review: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewOutput
): string[] {
  const findings = [
    "Phase 2J.31 is approval-gate only; it must not execute a gate, create, write, import, export, persist, save, or execute manifests.",
    "fixture_manifest_file_creation_dry_run_execution_gate_review_allowed is future approval/review eligibility only."
  ];
  if (sourceStatus !== "execution_gate_review_ok") {
  ...
  if (review.in_memory_execution_gate_review_preview.persisted) {
    findings.push("in_memory_execution_gate_review_preview_persisted_blocked");
  }
  return findings;
}

function summarizeDeniedActions(
  items: FixtureManifestFileCreationDryRunExecutionGateReviewApprovalItem[],
  records: SourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewApprovalRecord[]
): Record<string, number> {
  const itemIds = new Set(items.map((item) => item.execution_gate_review_item_id));
  return summarizeValues(
    records.filter((record) => itemIds.has(record.execution_gate_review_item_id)).flatMap((record) => record.requested_actions)
  );
}
...
function recommendedOwnerActions(
  ...
  const actions = [
    "Review approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review rows before any later approval/review phase.",
    "Do not execute a gate or create, write, export, import, save, persist, or execute any manifest in Phase 2J.31."
  ];
...
function nextSafeActions(
  ...
  if (sourceStatus !== "execution_gate_review_ok") {
    return [
      "Return to Phase 2J.30 fixture manifest file creation dry-run execution gate review before approval gating.",
  ...
  if (approvedCount > 0) {
    return [
      ...
      "Keep fixture_manifest_file_creation_dry_run_execution_gate_review_allowed as future eligibility only.",
...
function recommendedAction(status: z.infer<typeof ExecutionGateReviewApprovalStatusSchema>): string {
  if (status === "approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate_approval_review") {
...
function countApprovalStatus(
  items: FixtureManifestFileCreationDryRunExecutionGateReviewApprovalItem[],
  status: z.infer<typeof ExecutionGateReviewApprovalStatusSchema>
): number {
