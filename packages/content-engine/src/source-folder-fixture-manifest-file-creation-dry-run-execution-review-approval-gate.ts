...
import {
  ...
  type FixtureManifestFileCreationDryRunExecutionReviewItem,
  type SourceFolderFixtureManifestFileCreationDryRunExecutionReviewOutput
} from "./source-folder-fixture-manifest-file-creation-dry-run-execution-review.ts";

const EXECUTION_REVIEW_APPROVAL_GATE_ID =
  "phase-2j29-source-folder-fixture-manifest-file-creation-dry-run-execution-review-approval-gate";
...
const ExecutionReviewApprovalActionSchema = z.enum([
...
const ExecutionReviewApprovalStatusSchema = z.enum([
...
export const SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecordSchema = z.object({
  ...
  requested_actions: z.array(ExecutionReviewApprovalActionSchema).default([])
}).strict();

export const SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixtureSchema = z.object({
  fixture_version: z.literal(
    "source-folder-fixture-manifest-file-creation-dry-run-execution-review-approval-gate-smoke-v1"
  ),
  mode: z.literal("controlled_fixture_manifest_file_creation_dry_run_execution_review_approval_gate"),
  ...
  execution_review_approval_records: z.array(
    SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecordSchema
  ).min(1),
  notes: z.string().min(1)
}).strict();

export const FixtureManifestFileCreationDryRunExecutionReviewApprovalItemSchema = z.object({
  execution_review_item_id: z.string().min(1),
  ...
  source_execution_review_status: z.enum([
    "execution_review_ok",
    "needs_owner_review",
  ...
  approved_at: z.string().min(1).nullable(),
  approval_scope: z.string().min(1).nullable(),
  approval_status: ExecutionReviewApprovalStatusSchema,
...
export const SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateOutputSchema = z.object({
  fixture_manifest_file_creation_dry_run_execution_review_approval_gate_id: z.string().min(1),
  source_id: z.string().min(1),
  source_label: z.string().min(1),
  source_root: z.string().min(1),
  dry_run: z.boolean(),
  read_only: z.boolean(),
  source_execution_review_status: z.enum(["execution_review_ok", "execution_review_denied"]),
  reviewed_execution_review_items: z.number().int().min(0),
  ...
  approval_items: z.array(FixtureManifestFileCreationDryRunExecutionReviewApprovalItemSchema),
...
export type SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecord = z.infer<
  typeof SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecordSchema
>;
export type SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixture = z.infer<
  typeof SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixtureSchema
>;
export type FixtureManifestFileCreationDryRunExecutionReviewApprovalItem = z.infer<
  typeof FixtureManifestFileCreationDryRunExecutionReviewApprovalItemSchema
>;
export type SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateOutput = z.infer<
  typeof SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateOutputSchema
>;

export type SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateBatchResult = {
  provider: "fake_content_engine";
  fixture: SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixture;
  gates: SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateOutput[];
  reviewedExecutionReviewItems: number;
...
export function parseSourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixture(
  input: unknown
): SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixture {
  return SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixtureSchema.parse(input);
}

export function parseAndReviewSourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixture(input: {
...
}): SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateBatchResult {
  const approvalGateFixture = parseSourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateFixture(
    input.approvalGateFixtureInput
  );
  const executionReviewResult = parseAndReviewSourceFolderFixtureManifestFileCreationDryRunExecutionReviewFixture({
    executionReviewFixtureInput: input.executionReviewFixtureInput,
    executionGateFixtureInput: input.executionGateFixtureInput,
    approvalGateFixtureInput: input.fileCreationDryRunApprovalGateFixtureInput,
  ...
  const gates = executionReviewResult.reviews.map((review) =>
    reviewExecutionReviewApprovalGate(review, approvalGateFixture.execution_review_approval_records)
  );

  return {
    provider: "fake_content_engine",
    fixture: approvalGateFixture,
    gates,
    reviewedExecutionReviewItems: sum(gates.map((gate) => gate.reviewed_execution_review_items)),
...
export function reviewExecutionReviewApprovalGate(
  executionReview: SourceFolderFixtureManifestFileCreationDryRunExecutionReviewOutput,
  approvalRecords: SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecord[]
): SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateOutput {
  const sourceStatus = executionReview.fixture_manifest_file_creation_dry_run_execution_review_allowed
    ? "execution_review_ok"
    : "execution_review_denied";
  const duplicateIds = duplicateSuggestedFootageIds(executionReview.execution_review_item_reviews);
  const approvalItems = sourceStatus === "execution_review_ok"
    ? executionReview.execution_review_item_reviews.map((item) =>
        approvalItemForExecutionReview(
          item,
          approvalRecords.find((record) => record.execution_review_item_id === item.execution_review_item_id),
          duplicateIds
        )
      )
    : [];
  ...
  return SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalGateOutputSchema.parse({
    fixture_manifest_file_creation_dry_run_execution_review_approval_gate_id:
      `${EXECUTION_REVIEW_APPROVAL_GATE_ID}-${executionReview.source_id}`,
    source_id: executionReview.source_id,
    source_label: executionReview.source_label,
    source_root: executionReview.source_root,
    dry_run: executionReview.dry_run,
    read_only: executionReview.read_only,
    source_execution_review_status: sourceStatus,
    reviewed_execution_review_items: sourceStatus === "execution_review_ok"
      ? executionReview.execution_review_item_reviews.length
      : 0,
    approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate: approvedCount,
    ...
    approval_policy_findings: approvalPolicyFindings(sourceStatus, executionReview),
    ...
    target_fixture_manifest_path:
      ...
      )?.target_fixture_manifest_path ?? executionReview.target_fixture_manifest_path,
    metadata_write_allowed: false,
    manifest_write_allowed: false,
    fixture_manifest_write_allowed: executionReview.fixture_manifest_write_allowed,
    fixture_manifest_file_creation_gate_allowed: executionReview.fixture_manifest_file_creation_gate_allowed,
    fixture_manifest_file_creation_dry_run_allowed: executionReview.fixture_manifest_file_creation_dry_run_allowed,
    fixture_manifest_file_creation_execution_gate_allowed:
      executionReview.fixture_manifest_file_creation_execution_gate_allowed,
    fixture_manifest_file_creation_dry_run_execution_review_allowed:
      executionReview.fixture_manifest_file_creation_dry_run_execution_review_allowed,
    fixture_manifest_file_creation_dry_run_execution_gate_allowed: approvedCount > 0,
...
function approvalItemForExecutionReview(
  item: FixtureManifestFileCreationDryRunExecutionReviewItem,
  record: SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecord | undefined,
  duplicateIds: Set<string>
): FixtureManifestFileCreationDryRunExecutionReviewApprovalItem {
  const approvalRecord = record ?? defaultApprovalRecord(item.execution_review_item_id);
  ...
  return FixtureManifestFileCreationDryRunExecutionReviewApprovalItemSchema.parse({
    ...
    source_execution_review_status: item.execution_review_status,
    ...
    review_notes: [
      `Source execution review status: ${item.execution_review_status}.`,
      `Fixture manifest file creation dry-run execution review approval gate status: ${status}.`,
      "Approval is future execution-gate review eligibility only; no fixture manifest file is created, written, exported, imported, saved, persisted, or executed."
    ],
...
): SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecord {
  return {
...
function missingApprovalFieldsFor(
  record: SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecord
): string[] {
...
function blockingReasonsFor(
  ...
  if (item.execution_review_status === "blocked_execution_review") {
    reasons.push("source_execution_review_blocked_cannot_be_upgraded");
  }
  if (duplicateIds.has(item.suggested_footage_id)) reasons.push("duplicate_suggested_footage_id_blocked");
  for (const reason of item.blocking_reasons) reasons.push(`execution_review_${reason}`);
...
): z.infer<typeof ExecutionReviewApprovalStatusSchema> {
  if (item.execution_review_status === "incomplete_execution_review") return "incomplete_approval";
  if (blockingReasons.length > 0) return "blocked_approval";
  if (item.execution_review_status === "needs_owner_review") return "needs_owner_review";
  if (missingApprovalFields.length > 0) {
    if (item.execution_review_status === "execution_review_ok" && !record.approval_record_found) {
      return "needs_owner_review";
    }
    return "incomplete_approval";
  }
  if (item.execution_review_status !== "execution_review_ok") return "blocked_approval";
  return "approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate";
}

function duplicateSuggestedFootageIds(items: FixtureManifestFileCreationDryRunExecutionReviewItem[]): Set<string> {
...
function approvalPolicyFindings(
  sourceStatus: "execution_review_ok" | "execution_review_denied",
  review: SourceFolderFixtureManifestFileCreationDryRunExecutionReviewOutput
): string[] {
  const findings = [
    "Phase 2J.29 is approval-gate only; it must not create, write, import, export, persist, save, or execute manifests.",
    "fixture_manifest_file_creation_dry_run_execution_gate_allowed is future execution-gate review eligibility only."
  ];
  if (sourceStatus !== "execution_review_ok") {
    findings.push("Denied execution review cases produce zero approval items.");
  }
  if (review.source_root !== SAFE_FIXTURE_ROOT) findings.push("source_root_not_safe_fixture");
  if (review.in_memory_execution_preview.persisted) findings.push("in_memory_execution_preview_persisted_blocked");
  return findings;
}

function summarizeDeniedActions(
  items: FixtureManifestFileCreationDryRunExecutionReviewApprovalItem[],
  records: SourceFolderFixtureManifestFileCreationDryRunExecutionReviewApprovalRecord[]
...
function recommendedOwnerActions(
  ...
  if (sourceStatus !== "execution_review_ok") {
    return ["Keep denied execution review cases blocked; do not create approval items."];
  }
  const actions = [
    "Review approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate rows before any later execution gate.",
    "Do not create, write, export, import, save, persist, or execute any manifest in Phase 2J.29."
  ];
  if (items.some((item) => item.approval_status === "needs_owner_review")) {
    actions.push("Collect explicit owner approval before any later fixture-manifest file creation dry-run execution gate.");
  }
...
function nextSafeActions(
  ...
  if (sourceStatus !== "execution_review_ok") {
    return [
      "Return to Phase 2J.28 fixture manifest file creation dry-run execution review before approval gating.",
      "Do not create, export, import, write, save, persist, or execute any manifest."
    ];
  }
  if (approvedCount > 0) {
    return [
      "Treat approved rows as future fixture-manifest file creation dry-run execution gate review eligibility only.",
      "Keep fixture_manifest_file_creation_dry_run_execution_gate_allowed as future eligibility only.",
...
function recommendedAction(status: z.infer<typeof ExecutionReviewApprovalStatusSchema>): string {
  if (status === "approved_for_future_fixture_manifest_file_creation_dry_run_execution_gate") {
    return "Keep as future execution-gate review eligibility only; do not create or write a fixture manifest now.";
  }
...
function countApprovalStatus(
  items: FixtureManifestFileCreationDryRunExecutionReviewApprovalItem[],
  status: z.infer<typeof ExecutionReviewApprovalStatusSchema>
): number {
