...
import {
  ...
  parseAndReviewSourceFolderFixtureManifestFileCreationGateFixture
} from "../packages/content-engine/src/index.ts";

const fixturePath = "packages/content-engine/fixtures/source-folder-fixture-manifest-file-creation-gate-smoke.json";
...
const result = parseAndReviewSourceFolderFixtureManifestFileCreationGateFixture({
  fileCreationGateFixtureInput: JSON.parse(readFileSync(fixturePath, "utf8")),
...
const output = {
  ...
  fixture_manifest_file_creation_gate_cases: result.gates.length,
  reviewed_file_creation_approval_items: result.reviewedFileCreationApprovalItems,
  eligible_for_fixture_manifest_file_creation_dry_run_count:
    result.eligibleForFixtureManifestFileCreationDryRunCount,
  needs_owner_review_count: result.needsOwnerReviewCount,
  incomplete_file_creation_gate_count: result.incompleteFileCreationGateCount,
  blocked_file_creation_gate_count: result.blockedFileCreationGateCount,
  ...
  gate_summaries: result.gates.map((gate) => ({
    fixture_manifest_file_creation_gate_id: gate.fixture_manifest_file_creation_gate_id,
    source_id: gate.source_id,
    source_file_creation_approval_status: gate.source_file_creation_approval_status,
    reviewed_file_creation_approval_items: gate.reviewed_file_creation_approval_items,
    eligible_for_fixture_manifest_file_creation_dry_run:
      gate.eligible_for_fixture_manifest_file_creation_dry_run,
    needs_owner_review: gate.needs_owner_review,
    incomplete_file_creation_gate: gate.incomplete_file_creation_gate,
    blocked_file_creation_gate: gate.blocked_file_creation_gate,
  ...
  next_phase: "Phase 2J.25 Real Footage Source Folder Fixture Manifest File Creation Dry-Run Review"
};

if (
  output.provider !== "fake_content_engine" ||
  output.fixture_manifest_file_creation_gate_cases < 1 ||
  output.reviewed_file_creation_approval_items < 5 ||
  output.eligible_for_fixture_manifest_file_creation_dry_run_count < 1 ||
  output.needs_owner_review_count < 1 ||
  output.incomplete_file_creation_gate_count < 1 ||
  output.blocked_file_creation_gate_count < 1 ||
  ...
  throw new Error("real_footage_source_fixture_manifest_file_creation_gate_smoke_failed");
}
