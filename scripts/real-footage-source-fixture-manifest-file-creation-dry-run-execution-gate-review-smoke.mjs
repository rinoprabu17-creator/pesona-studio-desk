...
const result = parseAndReviewSourceFolderFixtureManifestFileCreationDryRunExecutionGateReviewFixture({
  executionGateReviewFixtureInput: readJson(fixturePath),
...
const output = {
  ...
  fixture_manifest_file_creation_dry_run_execution_gate_review_cases: result.reviews.length,
  reviewed_execution_review_approval_items: result.reviewedExecutionReviewApprovalItems,
  execution_gate_review_items: result.executionGateReviewItems,
  execution_gate_review_ok_count: result.executionGateReviewOkCount,
  needs_owner_review_count: result.needsOwnerReviewCount,
  incomplete_execution_gate_review_count: result.incompleteExecutionGateReviewCount,
  blocked_execution_gate_review_count: result.blockedExecutionGateReviewCount,
  ...
  in_memory_execution_gate_review_preview_count:
    result.inMemoryExecutionGateReviewPreviewCount,
  in_memory_execution_gate_review_preview_persisted_count:
    result.inMemoryExecutionGateReviewPreviewPersistedCount,
  ...
  review_summaries: result.reviews.map((review) => ({
    fixture_manifest_file_creation_dry_run_execution_gate_review_id:
      review.fixture_manifest_file_creation_dry_run_execution_gate_review_id,
    source_id: review.source_id,
    source_execution_review_approval_status: review.source_execution_review_approval_status,
    reviewed_execution_review_approval_items: review.reviewed_execution_review_approval_items,
    execution_gate_review_items: review.execution_gate_review_items,
    execution_gate_review_ok: review.execution_gate_review_ok,
    needs_owner_review: review.needs_owner_review,
    incomplete_execution_gate_review: review.incomplete_execution_gate_review,
    blocked_execution_gate_review: review.blocked_execution_gate_review,
    duplicate_id_count: review.duplicate_id_count,
    missing_required_field_count: review.missing_required_field_count,
    in_memory_execution_gate_review_preview_persisted:
      review.in_memory_execution_gate_review_preview.persisted,
    in_memory_execution_gate_review_preview_items:
      review.in_memory_execution_gate_review_preview.item_count,
  ...
  next_phase: "Phase 2J.31 Real Footage Source Folder Fixture Manifest File Creation Dry-Run Execution Gate Review Approval Gate"
};

if (
  output.provider !== "fake_content_engine" ||
  output.fixture_manifest_file_creation_dry_run_execution_gate_review_cases < 1 ||
  output.reviewed_execution_review_approval_items < 5 ||
  output.execution_gate_review_items < 1 ||
  output.execution_gate_review_ok_count < 1 ||
  output.needs_owner_review_count < 1 ||
  output.incomplete_execution_gate_review_count < 1 ||
  output.blocked_execution_gate_review_count < 1 ||
  ...
  output.in_memory_execution_gate_review_preview_persisted_count !== 0 ||
  ...
  throw new Error("real_footage_source_fixture_manifest_file_creation_dry_run_execution_gate_review_smoke_failed");
}
