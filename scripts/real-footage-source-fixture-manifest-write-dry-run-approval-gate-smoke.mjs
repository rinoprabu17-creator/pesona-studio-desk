...
import {
  ...
  parseAndReviewSourceFolderFixtureManifestWriteDryRunApprovalGateFixture
} from "../packages/content-engine/src/index.ts";

const fixturePath = "packages/content-engine/fixtures/source-folder-fixture-manifest-write-dry-run-approval-gate-smoke.json";
...
const result = parseAndReviewSourceFolderFixtureManifestWriteDryRunApprovalGateFixture({
...
const output = {
  ...
  fixture_manifest_write_dry_run_approval_gate_cases: result.gates.length,
  reviewed_write_plan_items: result.reviewedWritePlanItems,
  approved_for_future_fixture_manifest_file_creation_gate_count:
    result.approvedForFutureFixtureManifestFileCreationGateCount,
  ...
  gate_summaries: result.gates.map((gate) => ({
    fixture_manifest_write_dry_run_approval_gate_id: gate.fixture_manifest_write_dry_run_approval_gate_id,
    source_id: gate.source_id,
    source_write_plan_status: gate.source_write_plan_status,
    reviewed_write_plan_items: gate.reviewed_write_plan_items,
    approved_for_future_fixture_manifest_file_creation_gate:
      gate.approved_for_future_fixture_manifest_file_creation_gate,
  ...
  next_phase: "Phase 2J.24 Real Footage Source Folder Fixture Manifest File Creation Gate"
};

if (
  output.provider !== "fake_content_engine" ||
  output.fixture_manifest_write_dry_run_approval_gate_cases < 1 ||
  output.reviewed_write_plan_items < 5 ||
  output.approved_for_future_fixture_manifest_file_creation_gate_count < 1 ||
  ...
  throw new Error("real_footage_source_fixture_manifest_write_dry_run_approval_gate_smoke_failed");
}
