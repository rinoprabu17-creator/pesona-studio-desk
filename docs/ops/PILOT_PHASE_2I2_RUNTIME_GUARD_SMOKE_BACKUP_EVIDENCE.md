# Pilot Phase 2I.2 Runtime Guard Smoke Backup Evidence

## Purpose

This document records Phase 2I.2 owner-provided runtime guard smoke backup evidence for the controlled pilot stack `psd_pilot` on `pesona`.
...
- Record owner-provided Phase 2I.2 runtime guard smoke backup evidence.
...
```text
/srv/pesona-studio/backups/psd-pilot-phase-2i2-runtime-guard-smoke-backup-20260706T034554Z
```
...
| `channel-publish-state.txt` | `1551` bytes |
...
| `postgres-pilot-phase-2i2-runtime-guard-smoke.dump` | `170352` bytes |
| `route-status.txt` | `240` bytes |
| `runtime-baseline.txt` | `263` bytes |
| `runtime-db-counts.txt` | `327` bytes |
| `runtime-guard-smoke-assessment.txt` | `458` bytes |
| `SHA256SUMS.txt` | `1513` bytes |
| `storage-archive-list.txt` | `647` bytes |
| `storage-pilot-phase-2i2-runtime-guard-smoke.tgz` | `5952185` bytes |
...
- `postgres-pilot-phase-2i2-runtime-guard-smoke.dump`.
- `route-status.txt`.
- `runtime-baseline.txt`.
- `runtime-db-counts.txt`.
- `runtime-guard-smoke-assessment.txt`.
- `storage-archive-list.txt`.
- `storage-pilot-phase-2i2-runtime-guard-smoke.tgz`.

Checksum validation is PASS.

## Storage Archive Readability

Owner-provided evidence records that `storage-pilot-phase-2i2-runtime-guard-smoke.tgz` was readable.
...
Owner-provided evidence records that `postgres-pilot-phase-2i2-runtime-guard-smoke.dump` was readable through `pg_restore -l`.
...
This is Phase 2I.2 runtime guard smoke backup evidence only. It is not production backup and not cutover.
