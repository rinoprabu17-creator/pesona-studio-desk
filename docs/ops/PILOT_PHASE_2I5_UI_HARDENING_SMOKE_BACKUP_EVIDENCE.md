# Pilot Phase 2I.5 UI Hardening Smoke Backup Evidence

## Purpose

This document records Phase 2I.5 owner-provided UI hardening smoke backup evidence for the controlled pilot stack `psd_pilot` on `pesona`.

This is evidence intake only. Codex did not run commands on Lenovo or on the `pesona` server in this phase. This evidence does not authorize production backup, restore, restore dry-run, deployment, cutover, storage copy by Codex, public exposure, closeout, evidence log creation, checklist completion, actual publishing, scheduler/publisher automation, OpenAI live runtime, or production operation beyond the owner-run controlled pilot.

## Scope And Non-Goals

In scope:

- Record owner-provided Phase 2I.5 UI hardening smoke backup evidence.
...
| `postgres-pilot-phase-2i5-ui-hardening-smoke.dump` | `170352` bytes |
| `route-status.txt` | `240` bytes |
| `runtime-baseline.txt` | `263` bytes |
| `runtime-db-counts.txt` | `327` bytes |
| `SHA256SUMS.txt` | `1418` bytes |
| `storage-archive-list.txt` | `647` bytes |
| `storage-pilot-phase-2i5-ui-hardening-smoke.tgz` | `5952185` bytes |
| `ui-hardening-smoke-assessment.txt` | `613` bytes |
...
- `postgres-pilot-phase-2i5-ui-hardening-smoke.dump`.
- `route-status.txt`.
- `runtime-baseline.txt`.
- `runtime-db-counts.txt`.
- `storage-archive-list.txt`.
- `storage-pilot-phase-2i5-ui-hardening-smoke.tgz`.
- `ui-hardening-smoke-assessment.txt`.

Checksum validation is PASS.

## Storage Archive Readability

Owner-provided evidence records that `storage-pilot-phase-2i5-ui-hardening-smoke.tgz` was readable.
...
Owner-provided evidence records that `postgres-pilot-phase-2i5-ui-hardening-smoke.dump` was readable through `pg_restore -l`.
...
This is Phase 2I.5 UI hardening smoke backup evidence only. It is not production backup and not cutover.
...
Next safe phase: owner-reviewed manual checklist update smoke without actual publish, real manual publish proof evidence after actual manual posting, manual publish closeout evidence after readiness gaps are closed, or pilot backup restore dry-run planning in a separate isolated environment.
