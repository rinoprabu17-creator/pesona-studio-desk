# Phase 2G.4 Local Office Server Deployment Rehearsal

Branch: `phase-2g4-local-server-deployment-rehearsal`

Baseline tag: `phase-2g3-complete`

## Summary

Phase 2G.4 creates a docs-only deployment rehearsal and technician handoff pack for moving Pesona Studio Desk from laptop/dev workflow toward the local office Lenovo server.

This phase is planning and owner review material only. It does not perform server migration, backup, restore, deploy, container rebuild, DB restore, storage copy, or environment cutover.
...
- `docs/phase-2g4-local-server-deployment-rehearsal.md`
...
- No new DB query or DB write behavior added.
...
- Branch check passed: `phase-2g4-local-server-deployment-rehearsal`.
- Baseline tag check passed: `phase-2g3-complete` is reachable from `HEAD`.
- Initial `git status --short`: clean.
- `git diff --check`: passed.
- `npm run check`: passed.
- Docker Compose config resolved.
- Existing Postgres and Redis services were healthy.
- Docs safety grep found restricted terms only as warnings, non-goals, checklists, or safe evidence commands.
...
- README.local contains pre-existing OpenAI and test database placeholder examples; Phase 2G.4 only added links to new docs there.
- Docs intentionally mention destructive commands, public internet exposure, OpenAI, scheduler, publisher, upload, queue, worker daemon, and storage paths only as warnings, non-goals, checklists, or descriptions.
...
Phase 2G.4 is ready for owner review. Recommended next options are owner review of the cutover pack, technician evidence collection on the office server, or a separate owner-approved dry-run environment exercise.
