...
import { processOneOperationalMockupJob } from "../../../apps/web/src/operational-mvp-service.ts";

const config = {
  name: "mockup-worker",
  queueName: "ops:mockup-render",
  ...
  intervalMs: Number(process.env.MOCKUP_WORKER_POLL_INTERVAL_MS || "5000"),
  once: process.env.MOCKUP_WORKER_ONCE === "true"
};
...
async function processJob(): Promise<void> {
  const result = await processOneOperationalMockupJob();
  log("poll", result);
  if (config.once) {
